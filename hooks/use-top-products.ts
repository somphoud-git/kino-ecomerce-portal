...
export function useTopProducts(): UseTopProductsResult {
  ...
  const fetchTopProducts = useCallback(async () => {
    ...
    try {
      ...
      // Debug logging
      console.log('Top Products Query Results:', {
        topProductsCount: topProducts.length,
        newProductsCount: newProducts.length,
        topProducts: topProducts.map(p => ({ id: p.id, name: p.name, recommend: p.recommend })),
        newProducts: newProducts.map(p => ({ id: p.id, name: p.name, recommend: p.recommend }))
      })
      ...
      console.log('Final merged products:', uniqueProducts.map(p => ({ id: p.id, name: p.name, recommend: p.recommend })))
      
      setProducts(uniqueProducts)
    } catch (err) {
      console.error("Error fetching top and new products:", err)
      setError("Failed to fetch top and new products")
    } finally {
      setLoading(false)
    }
  }, [])
